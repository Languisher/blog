...
本文简单介绍了 C++ 中变量的基本类型和复合类型，主要参考 C++ Primer[^1] 第二章的内容。
...
**列表初始化**：如果希望指定的变量在自动转化时避免丢失信息（例如从 `double` 存储在 `float`），可以以下方式拒绝执行失去信息的初始化命令：
```cpp
...
**直接初始化和拷贝初始化**：使用 `=` 意味着拷贝初始化，等号右侧值被拷贝到等号左侧新创建的对象中；若不然（如 `int a(ld)` 和 `int c{ld}`）则是直接初始化。（很多时候变量并不是可以被拷贝的，建议使用直接初始化）
### 声明变量：在不同文件中使用同一变量
